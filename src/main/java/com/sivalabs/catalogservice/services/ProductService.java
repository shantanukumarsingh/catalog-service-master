...
public class ProductService {
    ...
    public List<Product> findAllProducts() {
    	log.info("Inside ProductService : findAllProducts");
        List<Product> products = productRepository.findAll();
        final Map<String, Integer> inventoryLevels = getInventoryLevelsWithFeignClient();
        log.info("Inside ProductService : findAllProducts : inventoryLevels : " + inventoryLevels +" \nsize : " + products.size() +  "\n : products :  "+products);
        ...
        log.info("Exit ProductService : findAllProducts : availableProducts : " + availableProducts.size() +  " : availableProducts :  "+availableProducts);
    ...
    public List<Product>  findProductByName(String strProductName) {
    	 log.info("Inside ProductService : findProductByName : " +strProductName );
    	List<Product> products = productRepository.findByName(strProductName);
        final Map<String, Integer> inventoryLevels = getInventoryLevelsWithFeignClient();
        log.info("Inside ProductService : findProductByName : products " +products +" : inventoryLevels : " + inventoryLevels );
        /*final Optional<Product> availableProducts = products.stream()
                .filter(p -> inventoryLevels.get(p.getCode()) != null && inventoryLevels.get(p.getCode()) > 0)
                .collect(Collectors.toList());*/
       final List<Product> availableProducts = products.stream()
                .filter(p -> inventoryLevels.get(p.getCode()) != null && inventoryLevels.get(p.getCode()) > 0)
                .collect(Collectors.toList());
        System.out.println("Inside ProductService : findProductByName : " + availableProducts);
        return availableProducts;
    }
}
